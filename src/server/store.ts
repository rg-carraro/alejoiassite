...
export function createRequest(raw:any){return transaction(()=>{
 ...
 const id=randomUUID(),createdAt=new Date().toISOString();const message=['Olá, AleJoias! Esta é minha seleção.','Solicitação: '+id,'Nome: '+name,'Telefone: '+phone,...items.map((i:any)=>`• ${i.name} (${i.id}) — ${i.variant} — ${i.quantity} un. × ${formatPrice(i.priceInCents)} = ${formatPrice(i.priceInCents*i.quantity)}`),'Subtotal: '+formatPrice(subtotalInCents)+' (entrega a confirmar).',...(items.some((i:any)=>i.demo)?['Contém peças/preços demonstrativos; confirmar valores reais.']:[]),'Solicitação sujeita à confirmação; não é compra ou reserva.',...(note?['Observação: '+note]:[])].join('\n');
