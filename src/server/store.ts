...
export function createRequest(raw:any,origin:string){return transaction(()=>{
 ...
 const id=randomUUID(),createdAt=new Date().toISOString(),pdfToken=randomBytes(32).toString('hex');
 const pdfUrl=new URL('/pedido/pdf',origin).href+'#'+id+'/'+pdfToken;
 const message=pdfUrl;
 const data={id,name,phone,note,items,subtotalInCents,createdAt,message,pdfToken,pdfUrl,whatsappUrl:'https://wa.me/5519988038395?text='+encodeURIComponent(message)};
