import {selectionPdf} from './order-pdf';
import type { APIRoute } from 'astro';
import {requestForPdf,authorized,login,logout,rateLimit,saveProduct,publicProducts,listProducts,productHistory,requests,requestStatus,createRequest,changePassword,dataDir,randomUUID} from './store';
...
export const ALL:APIRoute=async({request,params,cookies,url})=>{
 ...
 try{
 if(method==='GET'&&action==='auth-config')return json({mode:'password'});
