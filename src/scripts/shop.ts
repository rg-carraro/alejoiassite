...
function message(){const note=$<HTMLTextAreaElement>('#order-note')?.value.trim();return ['Olá, AleJoias! Gostaria de consultar esta seleção do protótipo:',...bag.map(i=>{const p=products.find(p=>p.id===i.id)!;return `• ${p.name} (${p.id}) — ${i.variant} — quantidade: ${i.quantity} — valor sob consulta`;}),'Subtotal e entrega: sob consulta.','São peças demonstrativas; por favor, confirme disponibilidade, medidas e valores.',...(note?['Observação: '+note]:[])].join('\n');}
function updateMessage(){const text=message();const preview=$<HTMLTextAreaElement>('#order-preview');if(preview)preview.value=text;const link=$<HTMLAnchorElement>('#whatsapp-order');if(link)link.href='https://wa.me/'+whatsapp+'?text='+encodeURIComponent(text);}
...
if(context?.registerTool){const lifecycle=new AbortController();try{Promise.resolve(context.registerTool({name:'read_shopping_bag',description:'Lê a seleção atual da sacola; não envia nem confirma um pedido.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute(input:unknown){if(!input||typeof input!=='object'||Array.isArray(input)||Object.keys(input).length)throw new Error('Use um objeto vazio.');return {items:bag.map(i=>({...i})),pricing:'sob consulta',confirmed:false};}},{signal:lifecycle.signal})).catch(()=>{});}catch{}window.addEventListener('pagehide',()=>lifecycle.abort(),{once:true});}
