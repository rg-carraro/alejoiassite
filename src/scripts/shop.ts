...
function message(){const note=$<HTMLTextAreaElement>('#order-note')?.value.trim();return ['Olá, AleJoias! Gostaria de consultar esta seleção do protótipo:',...bag.map(i=>{const p=products.find(p=>p.id===i.id)!;return `• ${p.name} (${p.id}) — ${i.variant} — quantidade: ${i.quantity} — unitário: ${formatPrice(p.priceInCents)} — total: ${formatPrice(p.priceInCents*i.quantity)}`;}),'Subtotal demonstrativo: '+formatPrice(subtotal())+'. Entrega a confirmar.','São peças e preços demonstrativos; por favor, confirme disponibilidade, medidas e valores.',...(note?['Observação: '+note]:[])].join('\n');}
