...
await page.goto('http://127.0.0.1:4322/catalogo');await page.locator('[data-add="DEMO-01"]').click();
...
const shared=await page.evaluate(()=>window.sharedPdf);assert.match(shared.name,/\.pdf$/);assert.ok(shared.size>1000);assert.match(shared.text,/Cliente Teste PDF/);
